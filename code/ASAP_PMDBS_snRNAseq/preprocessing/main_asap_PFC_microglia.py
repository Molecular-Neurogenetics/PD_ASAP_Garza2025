#!/bin/python
"""
Script for registering and processing microglia ASAP snRNA-seq samples for Prefrontal Cortex (PFC).

Workflow steps:
1. Load the region-specific sample sheet and post-QC samples.
2. Register samples with the trusTEr `Experiment` object.
3. Set output directories for merged microglia data.
4. Process cluster-level outputs, generating TE counts per cluster (gene counts disabled).

Notes:
- CellRanger quantification is skipped for subclustering.
- No sample filtering applied beyond post-QC.
...
# Merge and process microglia clusters
